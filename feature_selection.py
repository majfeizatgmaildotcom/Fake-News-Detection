...
# import dcor
# def dcor_fs(input_tr, output_tr, threshold, rank_num, fsmode='rank'):
...
#
#     input_tr = normalize_data_min_max_sk(input_tr)
#
#     col_list = input_tr.columns.values.tolist()
#     selected_features = []
#     dcor_matrix = pd.DataFrame(data=None, index=['rho'], columns=col_list)
#
#     for col in col_list:
#         rho = distance_correlation(input_tr.loc[:, col], output_tr)
#         dcor_matrix.loc['rho', col] = rho
#
#         if fsmode == "threshold":
...
#
#         if fsmode == "rank":
#             sortedf = np.argsort(np.abs(dcor_matrix.loc['rho', :])).tolist()
#             for f in range(rank_num):
#                 selected_features.append(col_list[sortedf[-f-1]])
#
#     return selected_features, dcor_matrix
